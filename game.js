(() => {
  ...
  function renderHighScores(values = loadHighScores()) { if (highScoresEl) highScoresEl.innerHTML = values.length ? values.map(value => `<li>${value}</li>`).join('') : '<li>No records</li>'; }
  ...
  function moveSnake() { if (!running || paused) return; direction = queuedDirection; const head = snake[0]; const next = { x: head.x + directions[direction].x, y: head.y + directions[direction].y }; if (next.x < 0 || next.x >= gridSize || next.y < 0 || next.y >= gridSize || isSnakeCell(next) || isEnemyCell(next)) { gameOver('Collision!'); return; } snake.unshift(next); if (sameCell(next, food)) { score += 1; level = Math.floor(score / 10) + 1; placeFood(); updateScoreboard(); beep(720, .08, 'square'); startTimers(); } else { snake.pop(); beep(220, .025, 'triangle'); } render(); }
  function updateEnemies() {
    if (!running || paused) return;
    const elapsed = Date.now() - enemyCycleStarted;
    if (!enemiesExploding && elapsed >= 5000) { enemiesExploding = true; enemies = enemies.map(enemy => ({ ...enemy, phase: 'exploding' })); setStatus('All 5 enemies explode!'); beep(120, .2, 'sawtooth'); render(); return; }
    if (enemiesExploding && elapsed >= 7000) { enemyCycleStarted = Date.now(); enemiesExploding = false; spawnEnemies(); setStatus('Enemies have respawned.'); render(); return; }
  ...
  function gameOver(message) { running = false; paused = false; clearTimers(); stopMusic(); saveScore(); setStatus(`${message} Game Over`); beep(180, .25, 'sawtooth'); if (pauseButton) pauseButton.disabled = true; render(); }
  function startGame() { resetGame(); running = true; paused = false; enemyCycleStarted = Date.now(); startTimers(); startMusic(); setStatus('Game in progress'); if (pauseButton) pauseButton.disabled = false; beep(520, .08, 'square'); }
  function togglePause() { if (!running) return; paused = !paused; setStatus(paused ? 'Paused' : 'Game in progress'); if (pauseButton) pauseButton.textContent = paused ? 'Resume' : 'Pause'; if (paused) stopMusic(); else startMusic(); beep(paused ? 260 : 520, .06, 'square'); }
  ...
  document.addEventListener('keydown', handleKey); document.querySelectorAll('[data-direction]').forEach(button => button.addEventListener('click', () => setDirection(button.dataset.direction))); startButton?.addEventListener('click', startGame); pauseButton?.addEventListener('click', togglePause); restartButton?.addEventListener('click', startGame); soundButton?.addEventListener('click', () => { soundEnabled = !soundEnabled; soundButton.textContent = soundEnabled ? 'Sound On' : 'Sound Off'; soundButton.setAttribute('aria-pressed', String(soundEnabled)); if (soundEnabled) { beep(520, .06, 'square'); startMusic(); } else stopMusic(); });
