(() => {
  ...
  let enemyCycleStarted = 0, enemiesExploding = false, soundEnabled = true, audioContext = null;
  ...
  function gameOver(message) { running = false; paused = false; clearTimers(); saveScore(); setStatus(`${message} 게임 오버`); beep(180, .25, 'sawtooth'); if (pauseButton) pauseButton.disabled = true; render(); }
  function startGame() { resetGame(); running = true; paused = false; enemyCycleStarted = Date.now(); startTimers(); setStatus('게임 진행 중'); if (pauseButton) pauseButton.disabled = false; beep(520, .08, 'square'); }
  function togglePause() { if (!running) return; paused = !paused; setStatus(paused ? '일시정지' : '게임 진행 중'); if (pauseButton) pauseButton.textContent = paused ? '계속하기' : '일시정지'; beep(paused ? 260 : 520, .06, 'square'); }
  function beep(frequency, duration, type) { if (!soundEnabled) return; try { audioContext ||= new (window.AudioContext || window.webkitAudioContext)(); const oscillator = audioContext.createOscillator(); const gain = audioContext.createGain(); oscillator.type = type; oscillator.frequency.value = frequency; gain.gain.setValueAtTime(.04, audioContext.currentTime); gain.gain.exponentialRampToValueAtTime(.001, audioContext.currentTime + duration); oscillator.connect(gain).connect(audioContext.destination); oscillator.start(); oscillator.stop(audioContext.currentTime + duration); } catch {} }
  ...
  document.addEventListener('keydown', handleKey); document.querySelectorAll('[data-direction]').forEach(button => button.addEventListener('click', () => setDirection(button.dataset.direction))); startButton?.addEventListener('click', startGame); pauseButton?.addEventListener('click', togglePause); restartButton?.addEventListener('click', startGame); soundButton?.addEventListener('click', () => { soundEnabled = !soundEnabled; soundButton.textContent = soundEnabled ? '소리 켜짐' : '소리 꺼짐'; soundButton.setAttribute('aria-pressed', String(soundEnabled)); if (soundEnabled) beep(520, .06, 'square'); });
